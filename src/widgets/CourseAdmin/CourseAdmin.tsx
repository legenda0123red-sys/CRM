  ...
  if (loading) {
    return (
      <div className="ml-122 flex min-h-[20vh] items-center justify-center">
        <p className="text-center font-bold text-gray-500 dark:text-white">
          Загрузка курсов...
        </p>
      </div>
    );
  ...
  return (
    <div className="grid grid-cols-1 gap-6 sm:grid-cols-2 xl:grid-cols-3">
      {visibleCourses.map((item, index) => {
        ...
        const level =
          levelMap[item.level] ?? levelMap.beginner;

        const format =
          formatMap[item.format] ?? formatMap.online;

        return (
          <div
            key={`${item.title}-${index}-${i18n.language}`}
